...
def adapt(config):
    ...
    def get_dataloader(
        dataset, batch_size, shuffle=False, pin_memory=True, collation=None
    ):
    ...
    validation_dataloaders = [
        source_validation_dataloader,
        target_validation_dataloader,
    ]
    ...
    if config.adaptation.self_paced:
        target_confidence_th = np.linspace(
            config.adaptation.target_confidence_th, 0.6, config.pipeline.epochs
        )
    else:
    ...
    tb_logger = TensorBoardLogger(
        save_dir=config.pipeline.tb.project_name,
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=os.path.join(save_dir, "checkpoints"), save_top_k=-1
    )
